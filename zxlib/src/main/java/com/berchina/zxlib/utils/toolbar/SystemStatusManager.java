package com.berchina.zxlib.utils.toolbar;
...
/**
 * Created by zx on 2017/3/25 14:28
 * 项目名称： SystemStatusManager
 * 类描述：
 * 使用方法：
 *1、拷贝状态栏管理类SystemStatusManager到你的项目中；
 *2、在你的BaseActivity中的onCreate方法中调用一下代码：
 *new SystemStatusManager(this).setTranslucentStatus(R.color.colorPrimary);//设置状态栏透明，参数为你要设置的颜色
 *3、将你项目中的所有页面的根布局加载一个属性： android:fitsSystemWindows="true"
 *注意：如果状态栏与标题栏中间有一条线导致有过渡感，那么在你项目的主题（style文件）中加入去掉线条的语句： <item name="android:windowContentOverlay">@null</item>
 * 备注
 */
